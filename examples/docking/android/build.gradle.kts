...
abstract class PrepareJniLibsTask : DefaultTask() {
    ...
    fun run() {
        ...
        abis.get().forEach { (target, abi) ->
            ...
            if (!src.exists()) {
                throw GradleException("Expected $src — did the ${buildType.get()} Kotlin/Native link task fail in :examples:docking?")
            }
...
android {
    namespace = "cn.enaium.imgui.example.docking"
    compileSdk = 36
    defaultConfig {
        applicationId = "cn.enaium.imgui.example.docking"
        minSdk = 24
        targetSdk = 36
        versionCode = 1
        versionName = "1.0"
...
androidComponents {
    onVariants { variant ->
        ...
        androidAbis.keys.forEach { target ->
            val linkTask = project(":examples:docking").tasks.named(
