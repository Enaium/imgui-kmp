...
abstract class PrepareJniLibsTask : DefaultTask() {
    ...
    fun run() {
        ...
        abis.get().forEach { (target, abi) ->
            ...
            if (!src.exists()) {
                throw GradleException("Expected $src — did the ${buildType.get()} Kotlin/Native link task fail in :examples:markdown?")
            }
...
android {
    namespace = "cn.enaium.imgui.example.markdown"
    compileSdk = 36
    defaultConfig {
        applicationId = "cn.enaium.imgui.example.markdown"
        minSdk = 24
        targetSdk = 36
        versionCode = 1
        versionName = "1.0"
...
androidComponents {
    onVariants { variant ->
        ...
        androidAbis.keys.forEach { target ->
            val linkTask = project(":examples:markdown").tasks.named(
